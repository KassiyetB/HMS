...
export async function getPatientById(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!result.rows.length) throw new AppError('Науқас табылмады', 404)
...
export async function createPatient(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.status(201).json({ success: true, data: result.rows[0], message: 'Науқас қосылды' })
  } catch (err) {
    next(err)
  }
}
...
export async function updatePatient(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!result.rows.length) throw new AppError('Науқас табылмады', 404)
    res.json({ success: true, data: result.rows[0], message: 'Науқас жаңартылды' })
  } catch (err) {
    next(err)
  }
}
...
export async function deletePatient(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!result.rows.length) throw new AppError('Науқас табылмады', 404)
    res.json({ success: true, message: `Науқас ${result.rows[0].name} жойылды` })
  } catch (err) {
    next(err)
  }
}
